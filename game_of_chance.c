...
int get_player_data() {
    ...
    if(fd == -1)
        return -1;
    read_bytes = _read(fd, &entry, sizeof(struct user));
...
void register_new_player() {
    ...
    fd = _open(DATAFILE, O_WRONLY|O_CREAT|O_APPEND, S_IRUSR|S_IWUSR);
    if(fd == -1)
        fatal("in register_new_player() while opening file");
    _write(fd, &player, sizeof(struct user));
    _close(fd);
...
void update_player_data() {
    int fd, i, read_uid;
    char burned_byte;
    fd = _open(DATAFILE, O_RDWR);
    if(fd == -1)
        fatal("in update_player_data() while opening file");
    _read(fd, &read_uid, 4);
    while(read_uid != player.uid) {
        for(i=0; i < sizeof(struct user) - 4; i++)
            _read(fd, &burned_byte, 1);
        _read(fd, &read_uid, 4);
    }
    _write(fd, &(player.credits), 4);
    _write(fd, &(player.highscore), 4);
    _write(fd, &(player.name), 100);
    _close(fd);
}
...
void show_highscore() {
    ...
    while(_read(fd, &entry, sizeof(struct user)) > 0) {
