...
window.Game5 = {
        ...
        setTimeout(() => {
            ...
            if (correct) {
                ...
                if (GameSystem.highLowState.streak >= GameSystem.highLowState.maxStreak) {
                    setTimeout(() => {
                        const multipliers = [1, 1.5, 2, 3, 5, 10];
                        const finalWin = Math.floor(GameSystem.highLowState.wager * multipliers[GameSystem.highLowState.streak]);
    ...
    cashOut: function() {
        const multipliers = [1, 1.5, 2, 3, 5, 10];
        const winAmount = Math.floor(GameSystem.highLowState.wager * multipliers[GameSystem.highLowState.streak]);
